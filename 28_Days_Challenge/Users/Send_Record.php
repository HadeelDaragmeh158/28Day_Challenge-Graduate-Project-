...
$sql1 = mysqli_query($dbConn,"select * from tasks_records where T_ID='$T_ID' AND U_ID='$U_ID'");
if (mysqli_num_rows($sql1)>0){


echo "<script language='JavaScript'>
			  alert ('You Already Send This Task Record !');
      </script>";	


echo '<script language="JavaScript">
            document.location="View_Tasks.php?C_ID='.$C_ID.'";
      </script>';
}
...
$sql123 = mysqli_query($dbConn,"select * from challenges where ID='$C_ID'");
$row123 = mysqli_fetch_array($sql123);
						
$Title=$row123['Title'];
$Picture=$row123['Picture'];




$sql456 = mysqli_query($dbConn,"select * from tasks where ID='$T_ID'");
$row456 = mysqli_fetch_array($sql456);
						
$Day_Number	=$row456['Day_Number'];
$Description=$row456['Description'];
...
$insert = mysqli_query($dbConn,"insert into tasks_records (Day_Number,T_ID,U_ID,Date,Picture_Before,Picture_After,Notes,Status) values ('$Day_Number','$T_ID','$U_ID','$Date','$file','$file2','$Notes','Pending')");


$Error = 'New Task Record Has Been Added !';

echo "<script language='JavaScript'>
			  alert ('New Task Record Has Been Added !');
      </script>";	
...
<div class="col-lg-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                     <h5>Add New Task Record | Day Number: <?php echo $Day_Number; ?></h5>
                     ...
                     <form method="post" action="Send_Record.php?T_ID=<?php echo $T_ID; ?>&C_ID=<?php echo $C_ID; ?>" class="form-horizontal" enctype="multipart/form-data">
                      
				<input type="hidden" name="C_ID" value="<?php echo $C_ID; ?>"/>
				<input type="hidden" name="U_ID" value="<?php echo $U_ID; ?>"/>
				<input type="hidden" name="T_ID" value="<?php echo $T_ID; ?>"/>
				<input type="hidden" name="Day_Number" value="<?php echo $Day_Number; ?>"/>
				
				
				

<center>
<img src="../Administrator/<?php echo $Picture; ?>" width="50%"/>
...
<br><br>
				...
				<div class="form-group"><label class="col-sm-2 control-label">Notes</label>
                                    ...
                                    <div class="col-sm-10"><input value="" type="text" required name="Notes"  class="form-control"></div>
                                </div>
...
<div class="form-group"><label class="col-sm-2 control-label">Date</label>
                                    ...
                                    <div class="col-sm-10"><input value="" type="date" required max="<?php echo date("Y-m-d"); ?>" name="Date" class="form-control"></div>
                                </div>
                                <div class="hr-line-dashed"></div>
...
<div class="form-group"><label class="col-sm-2 control-label">Picture Before</label>

                                    <div class="col-sm-10"><input value="" type="file"  accept="image/png, image/jpeg" name="file" class="form-control"></div>
                                </div>
                                <div class="hr-line-dashed"></div>



<div class="form-group"><label class="col-sm-2 control-label">Picture After</label>

                                    <div class="col-sm-10"><input value="" type="file"  accept="image/png, image/jpeg" name="file2" class="form-control"></div>
                                </div>
                                <div class="hr-line-dashed"></div>
